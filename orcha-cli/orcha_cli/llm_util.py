...
# xAI (Grok) speaks the OpenAI Chat Completions wire format, so GrokProvider translates this
# module's normalised (Anthropic-shaped) request/response to/from OpenAI framing.
...
# ----------------------------------------------------- OpenAI-format translation (xAI/Grok)
#
# xAI's API is OpenAI Chat-Completions compatible, so GrokProvider reuses these to translate
# the module's normalised (Anthropic-shaped) request/response to/from OpenAI framing.
...
def _to_openai_tool_choice(tool_choice: Optional[dict]) -> Optional[Any]:
    """Anthropic {"type":"tool","name":N} (a FORCED specific call) -> OpenAI
...
def _to_openai_message(m: dict) -> dict:
    ...
    blocks (used by describe_image) map to OpenAI multimodal parts. PDF 'document' blocks have
    no Chat-Completions equivalent and are dropped (describe_image fails open to "")."""
...
def _normalise_openai_stream_event(sse: dict, state: dict) -> Iterator[dict]:
    """Map one OpenAI stream chunk to zero+ normalised events. OpenAI streams a tool call's name
    only in its first ``tool_calls`` delta and the arguments as later fragments, and signals
...
class GrokProvider(Provider):
    """Live provider over xAI's OpenAI-compatible Chat Completions API, using only urllib.

    Vision (describe_image) is best-effort: images ride as OpenAI ``image_url`` data-URIs;
